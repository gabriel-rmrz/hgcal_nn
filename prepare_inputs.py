...
def prepare_fromSim(filename, suf, bins= [6,6,6], isPU=False):
  ...
  for i, ev in enumerate(simToReco_index): # looping over all the events
    ...
    s = ev[max_i,:]
    if DEBUG:
      print(f"np.sum(isPassScore0, axis=None): {np.sum(isPassScore0, axis=None)}")
      print(f"simToReco_en[i][isPassScore0]: {simToReco_en[i][isPassScore0]}")
      print(f"simTICLCandidate_raw_energy[i]: {simTICLCandidate_raw_energy[i]}")
      print(f"en_ratio: {en_ratio}")
      print(f"en_ratio: {max_i}")
      #for j, s in enumerate(ev[max_i]): #looping over the mergedTracksters 
      print(f"max_i: {max_i}")
      print(f"len(ev): {len(ev)}")
      print(f"ev: {ev}")
      print(f"s: {s}")
    j = max_i
    tsMPassed = isPassScore0[j]
    #print(tsMPassed)
    simMatched = s[tsMPassed]
    #simMatched = s[simToReco_score[i] < 0.35]
    mTs = tsLinkedInCand[i][simMatched]
    print(f"sum(stMPassed): {sum(tsMPassed)}")
    print(f"mTs: {mTs}")
    mTs_en = simToReco_en[i][j][tsMPassed]
    mTs = ak.flatten(tsLinkedInCand[i][simMatched])
    if len(mTs)==0:
      continue
    ...
    glob_pos = np.array([betaTs[i][mTs], bphiTs[i][mTs], bzTs[i][mTs]]).T
    ...
    glob_pos_cls = np.array([betaClsInTs, bphiClsInTs, bzClsInTs]).T
    ...
    fTsM_g3D_cls.append(np.array([np.histogramdd(rel_pos_cls, bins=bins,range=[[min_eta_cls,max_eta_cls],[min_phi_cls,max_phi_cls], [min_z_cls, max_z_cls]])[0],
                        np.histogramdd(rel_pos_cls, weights=np.asarray(corrected_enCls[i][vtxIds]), bins=bins, range=[[min_eta_cls,max_eta_cls],[min_phi_cls,max_phi_cls], [min_z_cls, max_z_cls]])[0]], dtype= np.int16))
  ...
  return deltaTsM_1D, fTsM_1D, fTsM_g3D, fTsM_g3D_cls, tTsM
...
def main():
  ...
  for suf in file_sufix:
    ...
    deltaTsM_1D, fTsM_1D, fTsM_g3D, fTsM_g3D_cls, tTsM = prepare_fromSim(f"data/histo_{suf}.root", suf, bins=bins, isPU=False)
